...
def train(args, model, rank, world_size, train_loader, optimizer, epoch, sampler=None):
    ...
    # Rank 0 prints the loss
    if rank == 0:
        train_loss = ddp_loss[0].item() / ddp_loss[1].item()
...
def test(model, rank, world_size, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # Sum up the loss for each batch
            loss = F.nll_loss(output, target, reduction="sum")
    ...
    if rank == 0:
        ...
        print(
            f"\nTest set: Average loss: {test_loss}, Accuracy: {ddp_loss[1]}/{ddp_loss[2]} ({accuracy:.0f}%)\n"
        )
...
def fsdp_main(rank, world_size, args):
    ...
    # Save the model
    if args.save_model:
        # Check if training is done on all ranks
        dist.barrier()
